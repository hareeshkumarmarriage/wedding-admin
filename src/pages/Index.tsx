...
const Index = () => {
  ...
  useEffect(() => {
    ...
    let attempts = 0;
    const focus = () => { attempts += 1; const element = document.getElementById(requested); if (element) { element.scrollIntoView({ behavior: "smooth", block: "start" }); return; } if (attempts < 10) window.setTimeout(focus, 100); };
    const timer = window.setTimeout(focus, 150); return () => window.clearTimeout(timer);
  }, [sections]);

  useEffect(() => {
    if (!settingsReady || wedding.introEnabled === false) return;
    let active = true;
    const readyKey = "wedding-loading-screen-ready-path-v1"; const currentPath = window.location.pathname; const introShownKey = `wedding-intro-overlay-shown-v1:${currentPath}`; const playEveryVisit = wedding.introVideoPlayMode === "always";
    const show = () => { if (!active) return; if (!playEveryVisit) { try { if (sessionStorage.getItem(introShownKey) === "1") return; sessionStorage.setItem(introShownKey, "1"); } catch {} } setShowIntro(true); };
    let ready = false; try { ready = sessionStorage.getItem(readyKey) === currentPath; } catch {}
    if (ready) { show(); return () => { active = false; }; }
    window.addEventListener("wedding-loading-finished", show); return () => { active = false; window.removeEventListener("wedding-loading-finished", show); };
  }, [settingsReady, wedding.introEnabled, wedding.introVideoPlayMode]);

  useEffect(() => {
    if (showIntro) return;
    const timer = window.setTimeout(() => {
      const key = "wedding-public-viewer-device-id-v1"; const namedKey = "wedding-public-viewer-name-v1"; let deviceId = localStorage.getItem(key);
  ...
  useEffect(() => {
    if (showIntro) return;
    const deviceId = localStorage.getItem("wedding-public-viewer-device-id-v1"); const visitorName = localStorage.getItem("wedding-public-viewer-name-v1"); if (!deviceId || !visitorName) return;
    ...
    const timer = window.setInterval(heartbeat, 60000); return () => window.clearInterval(timer);
  }, [showIntro]);
