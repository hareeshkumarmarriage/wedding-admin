...
const DriveVideos = () => {
  ...
  useEffect(() => {
    ...
    setError("");
    setSelectedIndex(null);
    getDriveVideosPage(event, eventRecord?.drive_folder_id).then((page) => {
      if (active) {
    ...
    return () => { active = false; };
  }, [event, eventRecord?.drive_folder_id, videosEnabled, navigate, valid, unlocked]);
  ...
  const loadMoreVideos = async () => {
    ...
    setError("");
    try {
      const page = await getDriveVideosPage(event, eventRecord?.drive_folder_id, videoPageToken);
