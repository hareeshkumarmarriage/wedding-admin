...
import { History, ShieldCheck, Users, X, RotateCcw, Rocket, Ban, KeyRound, RefreshCw } from "lucide-react";
import AdminControlCenterV2 from "./AdminControlCenterV2";
import { supabaseRest } from "@/lib/supabase";
import { changeAllEventSecurityCodes, changeEventSecurityCode, getEventSecurityCodes } from "@/lib/supabaseData";
...
type SecurityCode = { id:string; slug:string; title:string; code:string|null };
...
export default function AdminControlCenterV3(){
 ...
 const load=async()=>{if(!token)return;setBusy(true);try{
   ...
   try{setCodes((await getEventSecurityCodes(token))||[])}catch{setCodes([])}
 }catch(e){notify(e instanceof Error?e.message:"Unable to load management data")}finally{setBusy(false)}};
 useEffect(()=>{if(open)void load()},[open]);
 const publish=async()=>{setBusy(true);try{await supabaseRest<string>("rpc/admin_publish_revision",{token,method:"POST",body:{p_label:"Manual admin publish"}});notify("New publish revision created.");await load()}catch(e){notify(e instanceof Error?e.message:"Publish failed")}finally{setBusy(false)}};
 useEffect(()=>{
 ...
 const rollback=async(id:string)=>{if(!window.confirm("Roll back to this revision? The current state will be preserved as a revision before rollback."))return;setBusy(true);try{await supabaseRest<string>("rpc/admin_rollback_revision",{token,method:"POST",body:{p_revision_id:id}});notify("Rollback completed.");await load()}catch(e){notify(e instanceof Error?e.message:"Rollback failed")}finally{setBusy(false)}};
 const revoke=async(id:string)=>{if(!window.confirm("Revoke this admin session?"))return;setBusy(true);try{await supabaseRest<boolean>("rpc/admin_revoke_session",{token,method:"POST",body:{p_session_id:id}});notify("Session revoked.");await load()}catch(e){notify(e instanceof Error?e.message:"Session revoke failed")}finally{setBusy(false)}};
 const updateProfile=async(p:Profile)=>{const role=window.prompt("Role: admin, editor, or viewer",p.role);if(!role)return;const username=window.prompt("Username",p.username||"");if(username===null)return;const displayName=window.prompt("Display name",p.display_name||"");if(displayName===null)return;setBusy(true);try{await supabaseRest<boolean>("rpc/admin_update_profile",{token,method:"POST",body:{p_user_id:p.id,p_role:role,p_username:username,p_display_name:displayName}});notify("Profile updated.");await load()}catch(e){notify(e instanceof Error?e.message:"Profile update failed")}finally{setBusy(false)}};
 const changeCode=async(codeRow:SecurityCode)=>{const next=window.prompt(`New access code for ${codeRow.title}`,codeRow.code||"");if(next===null)return;const value=next.trim();if(value.length<4){notify("Security code must be at least 4 characters.");return}setBusy(true);try{await changeEventSecurityCode(token,{id:codeRow.id,slug:codeRow.slug},value);notify(`Security code updated for ${codeRow.title}.`);await load()}catch(e){notify(e instanceof Error?e.message:"Unable to update security code")}finally{setBusy(false)}};
 const changeAll=async()=>{const next=window.prompt("New access code for every event");if(next===null)return;const value=next.trim();if(value.length<4){notify("Security code must be at least 4 characters.");return}setBusy(true);try{const result=await changeAllEventSecurityCodes(token,value);notify(result.message||`${result.count} event codes updated.`);await load()}catch(e){notify(e instanceof Error?e.message:"Unable to update all security codes")}finally{setBusy(false)}};
 return <><AdminControlCenterV2/><button type="button" onClick={()=>setOpen(true)} className="fixed bottom-5 left-5 z-[90] inline-flex min-h-11 items-center gap-2 rounded-full border bg-background px-4 text-sm font-semibold shadow-xl"><ShieldCheck size={17}/>Backend controls</button>{open&&<div className="fixed inset-0 z-[100] bg-black/40 p-3 md:p-8"><section className="ml-auto h-full w-full max-w-5xl overflow-y-auto rounded-3xl border bg-background p-5 shadow-2xl"><div className="mb-5 flex items-center justify-between gap-3"><div><h2 className="text-xl font-semibold">Backend controls</h2><p className="text-sm text-muted-foreground">Publish revisions, rollback safely, manage sessions, profiles and event access codes.</p></div><Btn onClick={()=>setOpen(false)}><X size={17}/></Btn></div><div className="grid gap-5 lg:grid-cols-2"><Card title="Publishing & rollback"><div className="mb-3 flex gap-2"><Btn onClick={()=>void publish()} disabled={busy}><Rocket size={15}/>Create publish revision</Btn><Btn onClick={()=>void load()} disabled={busy}><History size={15}/>Refresh</Btn></div><div className="space-y-2">{revisions.length===0?<p className="text-sm text-muted-foreground">No revisions found.</p>:revisions.map(r=><div key={r.id} className="rounded-xl border p-3"><div className="flex items-start justify-between gap-3"><div><b>Version {r.version_no}</b><div className="text-xs text-muted-foreground">{r.label||"Untitled"} · {new Date(r.created_at).toLocaleString()}</div></div><Btn onClick={()=>void rollback(r.id)} disabled={busy}><RotateCcw size={14}/>Rollback</Btn></div></div>)}</div></Card><Card title="Event access codes"><div className="mb-3 flex gap-2"><Btn onClick={()=>void load()} disabled={busy}><RefreshCw size={15}/>Refresh codes</Btn><Btn onClick={()=>void changeAll()} disabled={busy}><KeyRound size={15}/>Set all codes</Btn></div><div className="space-y-2">{codes.length===0?<p className="text-sm text-muted-foreground">No event security codes returned.</p>:codes.map(c=><div key={c.id} className="flex items-center justify-between gap-3 rounded-xl border p-3"><div className="min-w-0"><b>{c.title}</b><div className="text-xs text-muted-foreground">{c.slug} · {c.code?"Code configured":"No code"}</div></div><Btn onClick={()=>void changeCode(c)} disabled={busy}><KeyRound size={14}/>Change</Btn></div>)}</div></Card><Card title="Admin sessions"><div className="space-y-2">{sessions.length===0?<p className="text-sm text-muted-foreground">No sessions returned.</p>:sessions.map(s=><div key={s.id} className="flex items-center justify-between gap-3 rounded-xl border p-3"><div className="min-w-0"><b className="truncate">{s.username||"Unknown user"}</b><div className="text-xs text-muted-foreground">{s.role} · {s.device||"Unknown device"} · {new Date(s.last_seen_at).toLocaleString()}</div></div>{s.revoked_at?<span className="text-xs text-red-600">Revoked</span>:<Btn danger onClick={()=>void revoke(s.id)} disabled={busy}><Ban size={14}/>Revoke</Btn>}</div>)}</div></Card><Card title="Admin profiles"><div className="space-y-2">{profiles.length===0?<p className="text-sm text-muted-foreground">No profiles returned.</p>:profiles.map(p=><div key={p.id} className="flex items-center justify-between gap-3 rounded-xl border p-3"><div className="min-w-0"><b>{p.display_name||p.username||p.id}</b><div className="text-xs text-muted-foreground">{p.username||"No username"} · {p.role}</div></div><Btn onClick={()=>void updateProfile(p)} disabled={busy}><Users size={14}/>Edit</Btn></div>)}</div></Card></div>{message&&<div role="status" className="fixed bottom-5 right-5 rounded-xl border bg-background p-3 text-sm shadow-xl">{message}</div>}</section></div>}</>;
}
