...
export async function getEvent(slug: string) {
  return (await getEvents()).find((item) => item.slug === slug) || null;
}
...
export async function getPendingGuestbook(token: string): Promise<GuestbookMessage[]> {
  if (!token) throw new Error("Admin session is missing.");
  return supabaseRest<GuestbookMessage[]>("guestbook", { token, query: "select=id,name,message,approved,moderation_status,featured,created_at&moderation_status=eq.pending&order=created_at.desc" });
}

export async function setGuestbookApproval(token: string, id: string, approved: boolean) {
  if (!token) throw new Error("Admin session is missing.");
  if (!id) throw new Error("Guestbook message ID is missing.");
  return supabaseRest("guestbook", { method: "PATCH", token, query: `id=eq.${encodeURIComponent(id)}`, body: { approved }, prefer: "return=representation" });
}

export async function deleteGuestbookMessage(token: string, id: string) {
  if (!token) throw new Error("Admin session is missing.");
  if (!id) throw new Error("Guestbook message ID is missing.");
...
export interface GuestRecord { id: string; name: string; phone: string | null; guest_group: string; relationship: string | null; invited: boolean; rsvp_id: string | null; guest_count: number; notes: string; created_at: string; updated_at: string; }
...
export async function getSiteSettings() {
  ...
  try { const rows = await supabaseRest<{ key: string; value: any }[]>("site_settings_public", { query: "select=key,value" }); return Object.fromEntries(rows.map((row) => [row.key, row.value])); } catch { return {}; }
}
...
export async function getHomepageSections() { if (!isSupabaseConfigured) return [] as HomepageSectionRecord[]; try { return await supabaseRest<HomepageSectionRecord[]>("homepage_sections_public", { query: "select=key,label,enabled,sort_order,updated_at&order=sort_order.asc" }); } catch { return []; } }
...
export async function getAdminEvents(token: string): Promise<EventRecord[]> { if (!token) throw new Error("Admin session is missing."); return supabaseRest<EventRecord[]>("events", { token, query: "select=id,slug,title,date,description,cover_image,cover_image_drive_id,drive_folder_id,photos_drive_folder_id,photos_drive_folder_id_2,videos_drive_folder_id,videos_drive_folder_id_2,sort_order,is_active,photos_enabled,videos_enabled,slideshow_enabled,qr_enabled,venue_name,venue_address,maps_url,created_at,updated_at&order=sort_order.asc" }); }
export async function getEventSecurityCodes(token: string) { const response = await fetch("/api/admin-event-code", { headers: { Authorization: `Bearer ${token}` } }); const data = await response.json().catch(() => ({})); if (!response.ok || !data.ok) throw new Error(data.error || "Unable to load event security codes."); return data.events as Array<{ id: string; slug: string; title: string; code: string | null }>; }
export async function changeAllEventSecurityCodes(token: string, code: string) { const response = await fetch("/api/admin-event-code", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ action: "set_all", code }) }); const data = await response.json().catch(() => ({})); if (!response.ok || !data.ok) throw new Error(data.error || "Unable to change all event security codes."); return data as { ok: true; message: string; count: number }; }
export async function getEventSecurityStatus() { const response = await fetch("/api/event-security-status", { cache: "no-store" }); return response.json() as Promise<{ ok: boolean; configured: boolean; missing: string[]; supabase: boolean; mediaDatabase: boolean; unlockSigning: boolean }>; }
export async function changeEventSecurityCode(token: string, event: Pick<EventRecord, "id" | "slug">, code: string) { if (!token) throw new Error("Admin session is missing."); const response = await fetch("/api/admin-event-code", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ event_id: event.id, slug: event.slug, code }) }); let data: { ok?: boolean; error?: string } = {}; try { data = await response.json(); } catch { /* ignore */ } if (!response.ok || !data.ok) throw new Error(data.error || "Unable to change the security code."); }
export async function createAdminEvent(token: string, event: Partial<EventRecord> & { slug: string; title: string; secret_code_hash: string }) { return supabaseRest("events", { method: "POST", token, body: event, prefer: "return=representation" }); }
export async function hashEventCode(code: string) { const data = new TextEncoder().encode(code.trim().toLowerCase()); const digest = await crypto.subtle.digest("SHA-256", data); return Array.from(new Uint8Array(digest)).map((b) => b.toString(16).padStart(2, "0")).join(""); }
export async function deleteAdminEvent(token: string, id: string) { return supabaseRest("events", { method: "DELETE", token, query: `id=eq.${encodeURIComponent(id)}` }); }
export async function updateAdminEvent(token: string, id: string, patch: Partial<EventRecord>) { if (!token) throw new Error("Admin session is missing."); return supabaseRest("events", { method: "PATCH", token, query: `id=eq.${encodeURIComponent(id)}`, body: patch, prefer: "return=representation" }); }
...
export async function getAdminAnalytics(token: string) {
  ...
  const topReactions = Object.values(reactions.reduce((acc: Record<string, any>, row) => { const key = `${row.event_id || ""}:${row.photo_id}`; acc[key] ||= { event_id: row.event_id, photo_id: row.photo_id, total: 0, heart: 0, love: 0, smile: 0 }; acc[key].total += 1; acc[key][row.reaction] += 1; return acc; }, {})).sort((a: any, b: any) => b.total - a.total).slice(0, 10);
