...
const CoupleSection = () => {
  ...
  const groomImage = driveFileIdUrl(content.groomImageDriveId) || groomFallback;
  const brideImage = driveFileIdUrl(content.brideImageDriveId) || brideFallback;
        ...
        <div className="grid md:grid-cols-5 gap-4 items-center justify-center place-items-center">
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="col-span-2 text-center">
            <div className="w-64 h-64 md:w-72 md:h-72 mx-auto rounded-full overflow-hidden border-4 border-wedding-blush shadow-lg mb-6">
              <img src={groomImage} alt={`${content.groomName || "Groom"} portrait`} className="w-full h-full object-cover" style={{ objectPosition: content.groomImagePosition || "center" }} loading="lazy" />
            </div>
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="col-span-2 text-center">
            <div className="w-64 h-64 md:w-72 md:h-72 mx-auto rounded-full overflow-hidden border-4 border-wedding-blush shadow-lg mb-6">
              <img src={brideImage} alt={`${content.brideName || "Bride"} portrait`} className="w-full h-full object-cover" style={{ objectPosition: content.brideImagePosition || "center" }} loading="lazy" />
            </div>
