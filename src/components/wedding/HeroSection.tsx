...
/** Convert the admin wedding date/time in its configured IANA timezone to an absolute timestamp. */
const parseWeddingDateTime = (dateValue: string, timeValue: string, timeZone = "Asia/Kolkata") => {
  ...
  try {
    // Determine the timezone offset at the requested wall-clock date/time, then
    // convert that wall-clock value to the corresponding UTC instant.
    ...
    const offsetMs = displayedUtc - wallClockUtc;
    const result = new Date(wallClockUtc - offsetMs);
    return Number.isFinite(result.getTime()) ? result : new Date(DEFAULT_WEDDING_DATE);
  } catch {
    const local = new Date(`${date}T${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}:${String(second).padStart(2, "0")}`);
    return Number.isFinite(local.getTime()) ? local : new Date(DEFAULT_WEDDING_DATE);
  }
};
