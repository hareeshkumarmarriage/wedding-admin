...
const parseWeddingDateTime = (dateValue: string, timeValue: string, timeZone = "Asia/Kolkata") => {
  ...
  if (match) { hour = Number(match[1]); minute = Number(match[2] || 0); second = Number(match[3] || 0); const meridiem = match[4] || ""; if (meridiem === "PM" && hour < 12) hour += 12; if (meridiem === "AM" && hour === 12) hour = 0; }
  ...
  try {
    const parts = new Intl.DateTimeFormat("en-US", { timeZone, year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit", hourCycle: "h23" }).formatToParts(new Date(wallClockUtc));
  ...
  } catch { return new Date(`${date}T${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}:${String(second).padStart(2, "0")}`); }
};
const useWeddingCountdown = (target: Date) => {
  ...
  useEffect(() => { setDifference(calculate()); const timer = window.setInterval(() => setDifference(calculate()), 1000); return () => window.clearInterval(timer); }, [targetMs]);
  const isPast = difference < 0; const totalSeconds = Math.floor(Math.abs(difference) / 1000);
...
type WeddingSettings = Record<string, unknown> & { groomName?: string; brideName?: string; date?: string; time?: string; timezone?: string; heroTitle?: string; description?: string; heroSubtitle?: string; heroImageDriveId?: string; countdownEnabled?: boolean };
...
const HeroSection = ({ settings }: { settings?: WeddingSettings }) => {
  ...
  return <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-wedding-cream">
    ...
    <div className="relative z-10 max-w-4xl text-center px-4">
      ...
      <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.4 }} className="font-display text-5xl md:text-7xl lg:text-8xl font-medium tracking-[0.1em] text-foreground mb-6">{bride}</motion.h1>
      {(content.description || content.heroSubtitle) && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.6 }} className="mx-auto mb-7 max-w-2xl font-body text-sm leading-7 text-muted-foreground md:text-base">{String(content.description || content.heroSubtitle)}</motion.p>}
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.8 }} className="font-accent text-xl md:text-2xl tracking-[0.2em] text-muted-foreground mb-8">{dateText} - {String(content.time || "08 . 59 . 00")}</motion.p>
      ...
      {content.countdownEnabled !== false && <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 1 }} className="flex justify-center gap-3 md:gap-6 lg:gap-10">
        {countdownItems.map(({ value, label }) => <div key={label} className="text-center"><div className="w-14 h-14 md:w-20 md:h-20 rounded-full border-2 border-primary/30 flex items-center justify-center mb-2 bg-background/50 backdrop-blur-sm"><span className="font-display text-xl md:text-3xl text-foreground">{String(value).padStart(2, "0")}</span></div><span className="font-body text-[9px] md:text-xs tracking-[0.15em] md:tracking-[0.2em] uppercase text-muted-foreground">{label}</span></div>)}
      </motion.div>}
    </div>
  </section>;
};
