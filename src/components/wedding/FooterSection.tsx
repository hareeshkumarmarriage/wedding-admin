  ...
  return (
    <footer className="py-12 bg-wedding-cream border-t border-border">
      <div className="wedding-container text-center">
        ...
        {links.some(({ key }) => content[key]) && <div className="mt-6 flex flex-wrap items-center justify-center gap-2">
          {links.filter(({ key }) => content[key]).map(({ key, label, icon: Icon }) => <button type="button" key={key} onClick={() => openLink(content[key])} aria-label={label} title={label} className="grid h-10 w-10 place-items-center rounded-full border border-primary/10 text-muted-foreground transition hover:bg-primary/5 hover:text-primary"><Icon size={17}/></button>)}
        </div>}
      </div>
    </footer>
  );
