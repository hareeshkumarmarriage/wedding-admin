...
const defaults: Required<SiteControlSettings> = {
  mode: "all",
  pages: { gallery: true, videos: true, upload: true, blog: true },
  maintenance: {
    enabled: false,
    title: "We'll be back soon",
    description: "We're preparing something special for you. Please check back shortly.",
    buttonText: "Back to Home",
  },
};

export default function PageAvailabilityGate({ page, children }: { page: string; children: React.ReactNode }) {
  const [state, setState] = useState<SiteControlSettings | null>(null);

  useEffect(() => {
    getSiteSettings()
      .then((settings) => {
        const siteControl = settings.siteControl as SiteControlSettings | undefined;
        setState({
          ...defaults,
          ...siteControl,
          pages: { ...defaults.pages, ...(siteControl?.pages || {}) },
          maintenance: { ...defaults.maintenance, ...(siteControl?.maintenance || {}) },
        });
      })
...
function MaintenanceScreen({ state }: { state: NonNullable<SiteControlSettings["maintenance"]> }) {
